...
describe('BoardStore', () => {
  ...
  describe('initialization', () => {
    ...
    it('loads existing data from file', () => {
      const existingDirectory = mkdtempSync(
        join(tmpdir(), 'kanbrawl-store-test-'),
      );
      const existingPath = join(existingDirectory, 'kanbrawl.json');
      writeFileSync(
        existingPath,
        JSON.stringify({
          columns: ['A', 'B'],
    ...
    it('fills in missing fields from partial file', () => {
      const partialDirectory = mkdtempSync(
        join(tmpdir(), 'kanbrawl-store-test-'),
      );
      const partialPath = join(partialDirectory, 'kanbrawl.json');
  ...
  describe('createTask', () => {
    ...
    it('throws for invalid column', () => {
      expect(() => store.createTask('Task', undefined, 'Nonexistent')).toThrow(
        /does not exist/,
      );
    });
  ...
  describe('moveTask', () => {
    ...
    it('throws for invalid column', () => {
      ...
      expect(() => store.moveTask(task.id, 'Nonexistent')).toThrow(
        /does not exist/,
      );
    });
  ...
  describe('updateTask', () => {
    ...
    it('throws for non-existent task', () => {
      expect(() => store.updateTask('nonexistent', { title: 'X' })).toThrow(
        /not found/,
      );
    });
